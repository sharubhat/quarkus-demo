...
data class UserEntity @BsonCreator constructor(
    ...
    companion object : ReactivePanacheMongoCompanion<UserEntity> {
        fun findUserById(id: ObjectId): Uni<UserEntity?> = findById(id)

        @Timed(name = "db_user_add", unit = MetricUnits.MILLISECONDS)
        fun saveUser(user: UserEntity) = user.persist<UserEntity>()
